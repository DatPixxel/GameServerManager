...
> Stand v3.31: `security_patches.py` ist nur noch ein Kompatibilitaetsmodul.
> Die aktiven Security-Implementierungen liegen direkt in `game_server_manager.py`,
> `security_utils.py`, `web_security.py` und `auth_manager.py`.
...
├── security_patches.py        # Legacy-Kompatibilitaet (kein aktiver Patch-Container)
...
```bash
cp security_utils.py web_security.py auth_manager.py security_patches.py /pfad/zu/gameservermanager/
```
...
# Schnelltest vor dem Start (empfohlen)
python selftest.py
